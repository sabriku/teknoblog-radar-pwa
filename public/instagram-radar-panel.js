(() => {
  ...
  function ensureStyle() {
    ...
    style.textContent = `
      ...
      .tb-ig-score-details{margin-top:9px;border:1px solid #fbcfe8;border-radius:10px;background:#fff7fb}.tb-ig-score-details summary{padding:8px 9px;color:#9d174d;font-size:10px;font-weight:900;cursor:pointer}.tb-ig-score-parts{display:flex;gap:5px;flex-wrap:wrap;padding:0 9px 9px}.tb-ig-score-parts span{padding:4px 6px;border-radius:7px;background:#fff;color:#475569;font-size:9px;font-weight:800}.tb-ig-rank-score{display:grid;justify-items:end;gap:2px}.tb-ig-rank-score small{color:#64748b;font-size:8px;font-weight:700}
      .tb-ig-digest{padding:16px}.tb-ig-digest-note{margin:0 0 13px;padding:10px 12px;border:1px solid #ddd6fe;border-radius:12px;background:#f5f3ff;color:#5b21b6;font-size:11px;line-height:1.5}.tb-ig-template-grid{display:grid;grid-template-columns:1fr 1fr;gap:14px}.tb-ig-template{overflow:hidden;border:1px solid #e2e8f0;border-radius:16px;background:#fff;box-shadow:0 5px 16px rgba(15,23,42,.05)}.tb-ig-template-head{display:flex;justify-content:space-between;align-items:center;gap:9px;padding:12px 13px;border-bottom:1px solid #e2e8f0}.tb-ig-template-head h3{margin:0;color:#111827;font:800 17px/1.2 'Fira Sans Condensed',sans-serif}.tb-ig-template-head button,.tb-ig-copy-all{border:0;border-radius:9px;background:#be185d;color:#fff;padding:8px 10px;font-size:10px;font-weight:900;cursor:pointer}.tb-ig-channel-basis{min-height:48px;margin:0;padding:10px 13px;border-bottom:1px solid #e2e8f0;background:#fff7fb;color:#64748b;font-size:10px;line-height:1.45}.tb-ig-template pre{max-height:360px;overflow:auto;margin:0;padding:14px;white-space:pre-wrap;font:12px/1.55 system-ui;color:#334155;background:#f8fafc}.tb-ig-digest-actions{display:flex;justify-content:flex-end;margin:12px 0}.tb-ig-ranked{display:grid;gap:7px}.tb-ig-ranked-channel{padding:10px;border-top:1px solid #e2e8f0;background:#fff}.tb-ig-rank{display:grid;grid-template-columns:34px 66px minmax(0,1fr) auto;align-items:center;gap:9px;padding:8px;border:1px solid #fce7f3;border-radius:12px;background:#fff}.tb-ig-rank>strong{display:grid;place-items:center;width:30px;height:30px;border-radius:50%;background:#fce7f3;color:#be185d}.tb-ig-rank img{width:66px;height:44px;border-radius:8px;object-fit:cover}.tb-ig-rank a{color:#1f2937;font-size:11px;font-weight:850;text-decoration:none;line-height:1.3}.tb-ig-rank small{display:block;margin-top:3px;color:#64748b;font-size:9px}.tb-ig-rank-score{white-space:nowrap;color:#9d174d;font-size:10px;font-weight:900}.tb-ig-template .tb-ig-rank{grid-template-columns:26px 48px minmax(0,1fr)}.tb-ig-template .tb-ig-rank>strong{width:24px;height:24px}.tb-ig-template .tb-ig-rank img{width:48px;height:36px}.tb-ig-template .tb-ig-rank-score{grid-column:3;justify-items:start}
  ...
  function digestView() {
    ...
    const fallbackItems = Array.isArray(digest.items) ? digest.items : [];
    const whatsappItems = Array.isArray(digest.whatsapp_items) ? digest.whatsapp_items : fallbackItems;
    const instagramItems = Array.isArray(digest.instagram_items) ? digest.instagram_items : fallbackItems;
    if (!whatsappItems.length && !instagramItems.length) return '<div class="tb-ig-empty">Bugün yayımlanmış yeterli Teknoblog haberi henüz bulunamadı.</div>';
    const whatsapp = digest.whatsapp_template || '';
    const instagram = digest.instagram_template || '';
    const rankingBasis = typeof digest.ranking_basis === 'object' && digest.ranking_basis ? digest.ranking_basis : { whatsapp: digest.ranking_basis || '', instagram: digest.ranking_basis || '' };
    const rankList = (items, channel) => `<div class="tb-ig-ranked tb-ig-ranked-channel">${items.map((item, index) => `<div class="tb-ig-rank"><strong>${index + 1}</strong><img src="${esc(item.image_url || fallbackImage(item))}" alt="" loading="lazy" referrerpolicy="no-referrer"><div><a href="${esc(item.url)}" target="_blank" rel="noopener noreferrer">${esc(item.title)}</a><small>${esc(item.digest_summary || '')}</small></div><span class="tb-ig-rank-score">${channel === 'whatsapp' ? 'WA' : 'IG'} ${Number(channel === 'whatsapp' ? item.whatsapp_channel_score || item.digest_score : item.instagram_channel_score || item.digest_score)}${item.read_signal_available ? `<small>${Number(item.total_clicks || 0)} gerçek tıklama</small>` : '<small>Editoryal tahmin</small>'}</span></div>`).join('')}</div>`;
    return `<div class="tb-ig-digest"><p class="tb-ig-digest-note"><b>İki ayrı seçim motoru kullanılıyor.</b> WhatsApp hızlı ve doğrudan bilgi değerini; Instagram görsel güç, merak, paylaşım ve kaydetme potansiyelini öne çıkarır. <strong>${Number(digest.overlap_count || 0)} ortak haber</strong> iki listede de kendi puanıyla üst sıraya girdi. ${Number(digest.actual_signal_count || 0) ? `<strong>${Number(digest.actual_signal_count)} haberde güncel etkileşim sinyali var.</strong>` : ''}</p><div class="tb-ig-template-grid"><section class="tb-ig-template"><div class="tb-ig-template-head"><h3>💬 WhatsApp Kanalı</h3><button data-ig-copy="${esc(whatsapp)}">Metni kopyala</button></div><p class="tb-ig-channel-basis">${esc(rankingBasis.whatsapp || '')}</p><pre>${esc(whatsapp)}</pre>${rankList(whatsappItems, 'whatsapp')}</section><section class="tb-ig-template"><div class="tb-ig-template-head"><h3>📣 Instagram Yayın Kanalı</h3><button data-ig-copy="${esc(instagram)}">Metni kopyala</button></div><p class="tb-ig-channel-basis">${esc(rankingBasis.instagram || '')}</p><pre>${esc(instagram)}</pre>${rankList(instagramItems, 'instagram')}</section></div><div class="tb-ig-digest-actions"><button class="tb-ig-copy-all" data-ig-copy="${esc(`${whatsapp}\n\n──────────\n\n${instagram}`)}">İki şablonu birlikte kopyala</button></div></div>`;
  }

  function scoreDetails(item) {
    if (state.tab !== 'story' || !item.story_score_breakdown) return '';
    const score = item.story_score_breakdown;
    return `<details class="tb-ig-score-details"><summary>Neden ${Number(item.story_score || 0)} puan?</summary><div class="tb-ig-score-parts"><span>Editoryal önem +${Number(score.editorial_importance || 0)}</span><span>Tazelik +${Number(score.freshness || 0)}</span><span>Performans +${Number(score.actual_performance || 0)}</span><span>Sosyal +${Number(score.social_fit || 0)}</span><span>Discover +${Number(score.discover_fit || 0)}</span><span>Görsel +${Number(score.visual || 0)}</span><span>Paylaşım +${Number(score.share_value || 0)}</span></div></details>`;
  }
