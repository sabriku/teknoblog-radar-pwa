(() => {
  if (window.__tbNewsApiFallbackInstalled) return;
  window.__tbNewsApiFallbackInstalled = true;
  ...
  async function fallbackResponse(reason = '') {
    const url = `/api/trend-overview?google_news=1&limit=40&fallback_for=recommendations&_=${Date.now()}`;
  ...
  window.fetch = async function tbFetchWithNewsFallback(input, init) {
