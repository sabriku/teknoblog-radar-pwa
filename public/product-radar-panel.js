(() => {
  const state = { items: [], brands: [], hours: '72', type: 'all', media: 'all', brand: 'all', loading: false, error: '', refreshedAt: '' };
  ...
  function render() {
    ...
    </style><div class="tb-pr-hero"><div class="tb-pr-hero-top"><div><h2>🛰️ Yeni Ürün Radarı</h2><p>Resmî üretici kaynaklarından yeni ürün ve hizmet duyuruları. Duyuruyla doğrulanmış resmî YouTube, Instagram ve X paylaşımları aynı kartta gösterilir.</p></div><button class="tb-pr-refresh" ${state.loading ? 'disabled' : ''}>↻ Şimdi eşzamanla</button></div><div class="tb-pr-controls"><div class="tb-pr-control"><label>Zaman</label><select data-pr-filter="hours">${options([['6','Son 6 saat'],['24','Son 24 saat'],['72','Son 3 gün'],['168','Son 7 gün']],state.hours)}</select></div><div class="tb-pr-control"><label>Tür</label><select data-pr-filter="type">${options([['product','Ürün'],['service','Hizmet']],state.type,'Tümü')}</select></div><div class="tb-pr-control"><label>Resmî medya</label><select data-pr-filter="media">${options([['youtube','YouTube videolu'],['instagram','Instagram paylaşımlı'],['x','X paylaşımlı']],state.media,'Tümü')}</select></div><div class="tb-pr-control"><label>Marka</label><select data-pr-filter="brand">${options(state.brands.map((brand) => [brand,brand]),state.brand,'Tüm markalar')}</select></div></div></div>
  ...
  async function load(force = false) {
    ...
    try {
      ...
      state.items = Array.isArray(data.items) ? data.items : []; state.brands = Array.isArray(data.brands) ? data.brands : state.brands; state.refreshedAt = data.refreshed_at || new Date().toISOString();
