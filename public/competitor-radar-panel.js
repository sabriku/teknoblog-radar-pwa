(() => {
  ...
  function render() {
    ...
    root.innerHTML = `<div class="tb-cr">${header()}<div class="tb-cr-notice" data-cr-notice>${state.loading ? 'Rakip kaynaklar taranıyor…' : `${items.length} benzersiz konu fırsatı gösteriliyor.`}</div><div class="tb-cr-bulk"><strong><span data-cr-selected-count>${state.selected.size}</span> konu seçildi</strong><button class="tb-cr-btn" data-cr-select-visible>Görünenleri seç</button><button class="tb-cr-btn" data-cr-clear>Temizle</button><button class="tb-cr-btn" data-cr-bulk-queue ${state.selected.size ? '' : 'disabled'}>＋ Toplu Yazılacaklara</button><button class="tb-cr-btn" data-cr-bulk-prompt ${state.selected.size ? '' : 'disabled'}>✨ Toplu prompt</button><button class="tb-cr-btn primary" data-cr-bulk-send ${state.selected.size ? '' : 'disabled'}>GPT’de aç</button></div>${items.length ? `<div class="tb-cr-grid">${items.map(card).join('')}</div>` : '<div class="tb-cr-empty">Bu filtrede rakip fırsatı bulunamadı.</div>'}</div><div class="tb-cr-modal" data-cr-modal hidden><div class="tb-cr-dialog" role="dialog" aria-modal="true" aria-label="GPT promptu"><div class="tb-cr-dialog-head"><h3>✨ Teknoblog özel GPT promptu</h3><button class="tb-cr-btn" data-cr-close>✕ Kapat</button></div><textarea data-cr-prompt-text></textarea><div class="tb-cr-dialog-actions"><button class="tb-cr-btn" data-cr-copy>⧉ Promptu kopyala</button><button class="tb-cr-btn primary" data-cr-open>GPT’ye gönder</button></div></div></div>`;
  }
  ...
  async function load(force = false) {
    ...
    try {
      ...
      state.items = data.items || []; state.summary = data.summary || {}; state.loaded = true; state.loading = false;
