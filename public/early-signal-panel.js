(() => {
  ...
  function card(item) {
    ...
    const queueData = esc(JSON.stringify({ candidate_id: news.id, title: item.cluster_name, url: news.url, source_name: news.source_name, image_url: news.image_url, status: 'new', priority: item.first_mover_score }));
