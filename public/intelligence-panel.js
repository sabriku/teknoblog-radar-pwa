(() => {
  ...
  const STRATEGY_KEY = 'tb_editorial_strategy';
  const sections = {
    command: ['Öncü Komuta', 'command'], today: ['Bugünün Kararı', 'summary'], early: ['Erken Sinyaller', 'early-signals'], clusters: ['Yükselen Konular', 'clusters'], lifecycle: ['Sinyal Akışı', 'lifecycle'], coverage: ['Teknoblog Kapsamı', 'coverage'],
  ...
  const groups = [
    { key: 'decide', label: 'Bugün & Sinyaller', icon: '⚡', views: ['command', 'today', 'early', 'clusters', 'lifecycle', 'coverage'] },
  ...
  const descriptions = {
    command: 'Hız, patlama ihtimali, kanıt, editoryal boşluk ve kaynak kapsamını seçtiğiniz stratejiye göre tek karar sırasına dönüştürür.',
  ...
  const state = { tab: initialView, strategy: localStorage.getItem(STRATEGY_KEY) || 'balanced', loading: false, data: {}, error: '', todayPosts: null, todaySummary: null, todayPostsError: '', queueSelected: new Set(), queueBusy: false, queueMessage: '', gaProperties: [] };
  ...
  const fmtNumber = (v) => new Intl.NumberFormat('tr-TR').format(Math.max(0, Number(v) || 0));
  const token = () => localStorage.getItem('tb_radar_cron_token') || localStorage.getItem('tb_cron_token') || '';

  async function get(section) {
    const strategy = section === 'command' ? `&strategy=${encodeURIComponent(state.strategy)}` : '';
    const response = await fetch(`/api/intelligence?section=${encodeURIComponent(section)}${strategy}&_=${Date.now()}`, { cache: 'no-store' });
  ...
  async function loadTodayPosts(force = false) {
    if (state.todayPosts && !force) return state.todayPosts;
    state.todayPostsError = '';
    try {
      const response = await fetch(`/api/teknoblog-latest?_=${Date.now()}`, { cache: 'no-store' });
      const data = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(data.error || `HTTP ${response.status}`);
      state.todayPosts = Array.isArray(data.items) ? data.items : [];
      state.todaySummary = data.analytics_summary || null;
    } catch (error) {
      state.todayPostsError = error?.message || String(error);
      state.todayPosts = [];
    }
  ...
  function command(data) {
    ...
    const strategyPicker = `<div class="tb-i-strategy"><div><b>Editoryal strateji</b><span>${esc(strategy.description || '')}</span></div><select data-strategy>${(data.strategies || []).map((item) => `<option value="${esc(item.key)}" ${item.key === strategy.key ? 'selected' : ''}>${esc(item.label)}</option>`).join('')}</select></div>`;
    const commandCards = data.items?.length ? `<div class="tb-i-command-grid">${data.items.slice(0, 16).map((item) => {
      const news = item.items?.[0] || item;
      const pack = item.editorial_package || {};
      const queueItem = { candidate_id: news.id, cluster_key: item.cluster_key, title: item.cluster_name || news.title, url: news.url, source_name: news.source_name, image_url: news.image_url, status: 'new', priority: item.strategy_score, references: pack.references || [] };
      const brief = `Teknoblog Öncü Radar briefi\nKonu: ${queueItem.title || ''}\nStrateji: ${strategy.label || ''}\nKarar puanı: ${item.strategy_score}\nAlarm: ${item.alert_level?.label || ''}\nKanıt: ${item.evidence?.label || ''}\nPatlama tahmini: %${item.burst_forecast?.probability || 0}, ${item.burst_forecast?.horizon_minutes || 0} dakika\nİlk yayın avantajı: ${item.first_publish_advantage?.label || ''}\nEditoryal boşluk: ${item.editorial_gap?.label || ''}\nAçı: ${pack.angle || ''}\nRisk: ${pack.risk_note || ''}\nReferanslar:\n${(pack.references || []).map((ref) => `- ${ref.source}: ${ref.url}`).join('\n')}`;
      return `<article class="tb-i-command-card level-${esc(item.alert_level?.key || 'gray')}">${img(news)}<div class="tb-i-body"><div>${pill(item.alert_level?.label || 'İzle', item.alert_level?.key === 'red' ? 'bad' : item.alert_level?.key === 'orange' ? 'hot' : item.alert_level?.key === 'blue' ? 'good' : '')}${pill(`Karar ${item.strategy_score}`)}${pill(`Patlama %${item.burst_forecast?.probability || 0}`)}${pill(item.evidence?.label || 'Kanıt bekleniyor')}</div><h3>${esc(item.cluster_name || news.title)}</h3><p>${esc(news.source_name || '')} · ${fmt(news.published_at || news.created_at)}</p><div class="tb-i-command-facts"><span><b>${item.burst_forecast?.horizon_minutes || 0} dk</b> tahmin ufku</span><span><b>${item.first_publish_advantage?.minutes || 0} dk</b> yayın avantajı</span><span><b>${item.editorial_gap?.score || 0}</b> boşluk puanı</span></div><p><b>Önerilen açı:</b> ${esc(pack.angle || '')}</p>${item.evidence?.caution ? `<p class="tb-i-risk">⚠️ ${esc(item.evidence.caution)}</p>` : ''}<div class="tb-i-actions"><a href="${esc(news.url || '#')}" target="_blank">Kaynağı aç</a><button data-queue='${esc(JSON.stringify(queueItem))}'>＋ Yazılacaklara</button><button data-slack='${esc(JSON.stringify(queueItem))}'>Slack’e gönder</button><button data-copy-brief="${esc(brief)}">Briefi kopyala</button></div></div></article>`;
    }).join('')}</div>` : empty('Seçilen stratejide güncel karar adayı bulunmuyor.');
    const coverageProblems = (data.source_coverage?.items || []).filter((item) => item.coverage_status !== 'covered').slice(0, 12);
    const coverage = coverageProblems.length ? `<div class="tb-i-table">${coverageProblems.map((item) => `<div><b>${esc(item.name)}</b>${pill(item.coverage_status === 'silent' ? 'Sessiz' : item.coverage_status === 'filtered' ? 'Filtrelendi' : 'Düşük görünürlük', item.coverage_status === 'silent' ? 'bad' : 'hot')}${pill(`${item.captured_24h || 0} alınan`)}<span>${esc(item.coverage_reason || '')}</span></div>`).join('')}</div>` : empty('Tüm aktif kaynaklar karar havuzuna içerik taşıyor.');
    const missed = data.missed_opportunities?.length ? `<div class="tb-i-table">${data.missed_opportunities.slice(0, 10).map((item) => `<div><b><a href="${esc(item.url)}" target="_blank">${esc(item.title)}</a></b>${pill(`Fırsat ${item.opportunity_score}`, 'hot')}${pill(item.queue_status)}<span>${esc(item.reason || '')}</span></div>`).join('')}</div>` : empty('Yüksek potansiyelli kaçırılmış fırsat bulunmuyor.');
    return `${strategyPicker}<div class="tb-i-metrics"><div><b>${s.urgent || 0}</b><span>hızlı karar</span></div><div><b>${s.burst_watch || 0}</b><span>patlama takibi</span></div><div><b>${s.open_advantages || 0}</b><span>açık öncülük penceresi</span></div><div><b>${s.sources_covered || 0}/${s.sources_active || 0}</b><span>kaynak kapsamı</span></div><div><b>${s.missed || 0}</b><span>kaçırılan fırsat</span></div><div><b>${s.published_today || 0}</b><span>bugünkü yayın</span></div></div><div class="tb-i-daily"><div><b>Günün Radar özeti</b><p>${esc(data.daily_summary || '')}</p></div><button data-copy-brief="${esc(data.daily_summary || '')}">Özeti kopyala</button></div><h3>Şimdi karar verilecekler</h3>${commandCards}<div class="tb-i-command-columns"><section><h3>Kaynak kapsama uyarıları</h3>${coverage}</section><section><h3>Kaçırılmış fırsatlar</h3>${missed}</section></div>`;
  }
  ...
  function todayPostsSidebar() {
    const items = state.todayPosts || [];
    const summary = state.todaySummary || {};
    const live = summary.available
      ? `<div class="tb-i-today-live"><small>● Bugünün canlı trafiği · ${fmt(summary.updated_at).split(' ').slice(-1)[0]}</small><span><b>${fmtNumber(summary.unique_visitors)}</b> tekil</span><span><b>${fmtNumber(summary.page_views)}</b> görüntülenme</span><span><b>${fmtNumber(summary.sessions)}</b> oturum</span></div>`
      : '<div class="tb-i-today-live pending">GA4 canlı trafik özeti bekleniyor…</div>';
    const body = state.todayPostsError
      ? `<div class="tb-i-today-empty">Bugünün yayınları alınamadı: ${esc(state.todayPostsError)}</div>`
      : items.length
        ? items.map((item) => {
          const analytics = item.analytics || {};
          const performance = analytics.available
            ? `<span class="tb-i-today-analytics"><b>👤 ${fmtNumber(analytics.unique_visitors)} tekil</b><b>👁 ${fmtNumber(analytics.page_views)} görüntülenme</b></span>`
            : '<span class="tb-i-today-analytics pending">GA4 verisi bekleniyor</span>';
          return `<article class="tb-i-today-post"><time>${fmt(item.published_at).split(' ').slice(-1)[0]}</time><a href="${esc(item.url)}" target="_blank" rel="noopener noreferrer">${esc(item.title)}</a>${performance}<button type="button" data-copy-published-url="${esc(item.url)}">🔗 Bağlantıyı kopyala</button></article>`;
        }).join('')
        : '<div class="tb-i-today-empty">Bugün henüz yayımlanmış haber bulunmuyor.</div>';
    return `<aside class="tb-i-today-sidebar"><header><div><b>📰 Bugün Teknoblog’da</b><span>${items.length} haber</span></div><button type="button" data-today-reload title="Listeyi yenile">↻</button></header>${live}<p>“Yazıldı” durumu tek tıklamayla kaydedilir. Bağlantı kopyalama yalnızca gerektiğinde hızlı erişim içindir.</p><div class="tb-i-today-list">${body}</div></aside>`;
  }
  function queue(items) {
    ...
    const automation = state.data.queue?.automation || {};
    const automationText = automation.last_checked_at
      ? `Son kontrol ${fmt(automation.last_checked_at)} · ${automation.checked || 0} görev tarandı${automation.matched ? ` · ${automation.matched} yayın bulundu` : ''}`
      : 'Otomatik yayın kontrolü ilk çalışmasını bekliyor';
    const toolbar = `<div class="tb-i-queue-automation"><span>🔎 Teknoblog yayınları 15 dakikada bir otomatik kontrol edilir. ${esc(automationText)}</span><button data-queue-reconcile ${state.queueBusy ? 'disabled' : ''}>Şimdi kontrol et</button></div><div class="tb-i-queue-toolbar"><div><button data-queue-select-open ${state.queueBusy ? 'disabled' : ''}>☑ Açıkların tümünü seç</button><button data-queue-clear ${selected.length ? '' : 'disabled'}>Seçimi kaldır</button><b>${selected.length} seçili</b></div><div>${statuses.map(([status, label]) => `<button data-bulk-status="${status}" ${selected.length && !state.queueBusy ? '' : 'disabled'}>${label === 'Onayla' ? '✓ Toplu onayla' : label}</button>`).join('')}</div></div>${state.queueMessage ? `<div class="tb-i-queue-message">${esc(state.queueMessage)}</div>` : ''}`;
  ...
  function content() {
    ...
    if (state.tab === 'command') return command(data);
  ...
  function render() {
    ...
    root.innerHTML = `<style>
      ...
      .tb-i-today-live{display:grid;grid-template-columns:repeat(3,1fr);gap:5px;margin:10px;padding:8px;border:1px solid #fdba74;border-radius:11px;background:linear-gradient(135deg,#fff7ed,#eff6ff);font-size:9px;color:#64748b}.tb-i-today-live small{grid-column:1/-1;font-weight:900;color:#9a3412}.tb-i-today-live span{padding:5px;border-radius:7px;background:#fff}.tb-i-today-live span b{display:block;font-size:13px;color:#0f172a}.tb-i-today-live.pending{display:block;font-weight:800}.tb-i-today-analytics{display:flex;gap:5px;flex-wrap:wrap}.tb-i-today-analytics b{padding:3px 5px;border-radius:999px;background:#ecfdf5;color:#047857;font-size:9px}.tb-i-today-analytics b+ b{background:#eff6ff;color:#1d4ed8}.tb-i-today-analytics.pending{font-size:9px;font-weight:800;color:#94a3b8}.tb-i-queue-automation{display:flex;align-items:center;justify-content:space-between;gap:10px;margin-bottom:9px;padding:9px 11px;border:1px solid #bae6fd;border-radius:12px;background:#f0f9ff;color:#075985;font-size:10px;font-weight:800}.tb-i-queue-automation button{flex:0 0 auto;border:1px solid #38bdf8;border-radius:8px;background:#fff;color:#0369a1;padding:6px 9px;font-size:10px;font-weight:900;cursor:pointer}.tb-i-queue-toolbar{position:sticky;top:8px;z-index:4;display:flex;justify-content:space-between;gap:8px;flex-wrap:wrap;margin-bottom:12px;padding:10px;border:1px solid #c7d2fe;border-radius:13px;background:rgba(255,255,255,.96);box-shadow:0 5px 16px rgba(15,23,42,.08);backdrop-filter:blur(8px)}.tb-i-queue-toolbar>div{display:flex;gap:6px;align-items:center;flex-wrap:wrap}.tb-i-queue-toolbar button{border:1px solid #cbd5e1;border-radius:9px;background:#fff;color:#334155;padding:7px 9px;font-size:10px;font-weight:900;cursor:pointer}.tb-i-queue-toolbar [data-bulk-status="approved"]{border-color:#f97316;color:#c2410c}.tb-i-queue-toolbar button:disabled,.tb-i-queue-automation button:disabled{opacity:.45;cursor:not-allowed}.tb-i-queue-message{margin:-4px 0 10px;padding:8px 10px;border-radius:9px;background:#ecfdf5;color:#166534;font-size:11px;font-weight:900}.tb-i-queue-check{display:inline-flex;align-items:center;gap:4px;font-size:10px;font-weight:900;color:#475569}.tb-i-queue-check input{width:17px;height:17px;accent-color:#f04a0a}
      .tb-i-strategy,.tb-i-daily{display:flex;justify-content:space-between;align-items:center;gap:12px;border:1px solid #c7d2fe;border-radius:15px;padding:13px;background:linear-gradient(135deg,#eef2ff,#fff)}.tb-i-strategy>div{display:grid;gap:3px}.tb-i-strategy span{font-size:11px;color:#64748b}.tb-i-strategy select{min-width:180px;border:1px solid #818cf8;border-radius:10px;padding:9px;background:#fff;color:#312e81;font-weight:900}.tb-i-command-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(300px,1fr));gap:12px}.tb-i-command-card{border:1px solid #e2e8f0;border-left-width:6px;border-radius:17px;overflow:hidden;background:#fff;box-shadow:0 5px 16px rgba(15,23,42,.06)}.tb-i-command-card.level-red{border-left-color:#dc2626}.tb-i-command-card.level-orange{border-left-color:#f97316}.tb-i-command-card.level-yellow{border-left-color:#eab308}.tb-i-command-card.level-blue{border-left-color:#2563eb}.tb-i-command-card.level-gray{border-left-color:#94a3b8}.tb-i-command-card h3{font-size:18px;line-height:1.25;margin:8px 0}.tb-i-command-facts{display:grid;grid-template-columns:repeat(3,1fr);gap:6px;margin:10px 0}.tb-i-command-facts span{display:grid;gap:2px;border-radius:9px;padding:8px;background:#f8fafc;color:#64748b;font-size:9px}.tb-i-command-facts b{font-size:13px;color:#0f172a}.tb-i-risk{border-radius:9px;padding:8px;background:#fff7ed;color:#9a3412!important}.tb-i-daily{margin:12px 0;background:linear-gradient(135deg,#ecfdf5,#fff)}.tb-i-daily p{margin:4px 0 0;color:#475569;font-size:12px;line-height:1.5}.tb-i-daily button{flex:0 0 auto;border:1px solid #86efac;border-radius:9px;background:#fff;color:#166534;padding:8px 10px;font-weight:900;cursor:pointer}.tb-i-command-columns{display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-top:16px}.tb-i-command-columns>section{min-width:0}
      @media(max-width:1050px){.tb-i-queue-layout,.tb-i-command-columns{grid-template-columns:1fr}.tb-i-today-sidebar{position:static}.tb-i-today-list{max-height:380px}}@media(max-width:800px){.tb-i-groups{grid-template-columns:repeat(2,minmax(0,1fr))}.tb-i-hero{padding:16px}.tb-i-content{padding:12px}.tb-i-table>div,.tb-i-form{grid-template-columns:1fr}.tb-i-table nav{grid-column:1}.tb-i-grid,.tb-i-command-grid{grid-template-columns:1fr}.tb-i-strategy,.tb-i-daily{align-items:stretch;flex-direction:column}.tb-i-strategy select{width:100%}}@media(max-width:460px){.tb-i-groups{grid-template-columns:1fr}.tb-i-group{justify-content:flex-start}.tb-i-command-facts{grid-template-columns:1fr}}
  ...
  async function load(tab = state.tab, force = false) {
    state.tab = sections[tab] ? tab : 'command'; state.error = '';
    localStorage.setItem(VIEW_KEY, state.tab);
    if (state.data[tab] && !force) { render(); return; }
    state.loading = true; render();
    try {
      const sectionRequest = get(sections[tab][1]);
      const todayRequest = tab === 'queue' ? loadTodayPosts(force) : Promise.resolve();
      state.data[tab] = await sectionRequest;
      await todayRequest;
  ...
  document.addEventListener('click', async (event) => {
    ...
    const reconcileQueue = event.target.closest('[data-queue-reconcile]');
    if (reconcileQueue) {
      reconcileQueue.disabled = true;
      state.queueBusy = true; state.queueMessage = 'Teknoblog yayınları kontrol ediliyor…'; render();
      try {
        const result = await post({ action: 'reconcile_queue_publications' }, true);
        state.queueMessage = result.matched ? `${result.matched} görev Teknoblog yayınıyla eşleşti ve Yazıldı yapıldı.` : 'Yeni yayımlanmış eşleşme bulunmadı.';
        const message = state.queueMessage;
        await load('queue', true);
        state.queueMessage = message;
      } catch (e) { state.queueMessage = ''; alert(e.message); }
      finally { state.queueBusy = false; render(); }
      return;
    }
    ...
    if (publishedCopy) {
      try {
        const url = publishedCopy.dataset.copyPublishedUrl || '';
        await navigator.clipboard.writeText(url);
        publishedCopy.textContent = '✓ Kopyalandı';
        setTimeout(() => { publishedCopy.textContent = '🔗 Bağlantıyı kopyala'; }, 1800);
      } catch (e) { alert(`Kopyalama hatası: ${e.message || e}`); }
      return;
    }
    const queued = event.target.closest('[data-queue],[data-add-queue]');
    if (queued) { try { await post({ action: 'queue_upsert', ...JSON.parse(queued.dataset.queue || queued.dataset.addQueue) }); queued.textContent = '✓ Yazılacaklarda'; queued.disabled = true; queued.closest('article, .tb-news-card, .tb-i-card')?.classList.add('tb-queue-added'); state.data.queue = null; } catch (e) { alert(e.message); } return; }
    ...
    if (slack) {
      ...
      try {
        ...
        slack.textContent = '✓ Slack’e gönderildi'; slack.closest('article, .tb-news-card, .tb-i-card')?.classList.add('tb-queue-added'); state.data.queue = null;
  ...
  document.addEventListener('change', (event) => {
    ...
    if (strategy) {
      ...
      localStorage.setItem(STRATEGY_KEY, state.strategy);
      state.data.command = null;
      load('command', true);
      return;
    }
  ...
  function start() {
    render();
    if (location.hash === '#decision-center' || ['#decision', '#editorial', '#intelligence', '#ops'].includes(location.hash)) load(state.tab);
    window.setInterval(async () => {
      if (document.visibilityState !== 'visible' || state.tab !== 'queue') return;
      await loadTodayPosts(true);
      render();
    }, 60 * 1000);
  }
