...
export default async function handler(req, res) {
  try {
    ...
    for (const cluster of clusterMap.values()) {
      ...
      const affiliatePotentialScore = recommendationType(cluster.clusterName, avgDiscover) === 'satın_alma' ? Math.max(65, avgTraffic) : Math.round(avgTraffic * 0.55);
      ...
      const clusterRow = {
        ...
        recommendation_type: recommendationType(cluster.clusterName, avgDiscover),
      ...
      if (clusterError) {
        debug.push({ cluster: cluster.clusterName, status: 'cluster_error', error: clusterError.message });
        continue;
      }
      clusterUpserts += 1;
