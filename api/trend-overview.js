...
const STRONG_TECH_INCLUDE_PATTERNS = [
  ...
  /\bgenie\b/i, /\bomni\b/i, /\bspark\b/i
];
...
const TECH_EXCLUDE_PATTERNS = [
  ...
  /\bborsa\b/i, /\bprime video\b/i, /\bnetflix\b/i, /\bdizi\b/i, /\bsezon\b/i,
  /\bbölüm\b/i, /\bkimdir\b/i, /\bsevgilisi\b/i, /\bsurvivor\b/i, /\bthe boys\b/i,
  /\bçılgın sayısal\b/i, /\bşok kataloğu\b/i, /\bbankalar açık mı\b/i,
...
const TURKEY_TECH_SOURCE_PATTERNS = [
  /shiftdelete/i, /donanımhaber/i, /webtekno/i, /tamindir/i, /log\.com\.tr/i,
  /teknoblog/i, /webrazzi/i, /chip online/i, /hardware plus/i,
  /google trends tr/i, /tr 4s teknoloji/i, /tr 24s teknoloji/i, /tr 48s teknoloji/i
];

function clamp(value, min, max) { return Math.min(max, Math.max(min, value)); }

function normalizeTopic(value = '') {
  return String(value || '').toLowerCase().replace(/[^a-z0-9çğıöşü\s]/gi, ' ').replace(/\s+/g, ' ').trim();
}
...
function windowHours(windowValue = '24h') { return Number(String(windowValue).replace(/[^0-9]/g, '')) || 24; }
...
function buildSparkline(signals = [], selectedWindow = '24h') {
  ...
  for (let i = 0; i < bucketCount; i += 1) {
    ...
    buckets.push({ ts: new Date(bucketStart).toISOString(), label: formatBucketLabel(new Date(bucketStart), bucketHours), count: 0 });
  }
...
function hasStrongTech(text = '') { return STRONG_TECH_INCLUDE_PATTERNS.some((pattern) => pattern.test(String(text || ''))); }
function hasWeakTech(text = '') { return WEAK_TECH_INCLUDE_PATTERNS.some((pattern) => pattern.test(String(text || ''))); }
function looksExcluded(text = '') { return TECH_EXCLUDE_PATTERNS.some((pattern) => pattern.test(String(text || ''))); }

function hasTurkeyTechSignal(cluster = {}) {
  const sourceText = [
    cluster.country_code,
    cluster.market_scope,
    ...(Array.isArray(cluster.signal_sources) ? cluster.signal_sources : []),
  ...
  return Number(cluster.turkey_interest_score || 0) >= 50 ||
    /\bTR\b/i.test(sourceText) || /turkey|türkiye/i.test(sourceText) ||
...
function isTechCluster(cluster = {}) {
  ...
  const text = [
    ...
    ...(Array.isArray(cluster.summary?.sample_topics) ? cluster.summary.sample_topics : []),
    ...(Array.isArray(cluster.signal_topics) ? cluster.signal_topics : []),
  ...
  return hasWeakTech(text) && (linkedNews.length >= 1 || Number(cluster.window_signal_count || 0) >= 1);
}
...
function decorateCluster(cluster = {}, signalGroup = {}, selectedWindow = '24h') {
  ...
  return {
    ...
    signal_sources: [...new Set(signals.map((item) => item.source_name).filter(Boolean))],
    signal_topics: [...new Set(signals.map((item) => item.topic_text).filter(Boolean))],
